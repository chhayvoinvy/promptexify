...
-- Overload helper to accept TEXT ids (fallback)
CREATE OR REPLACE FUNCTION can_access_post(post_id TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  RETURN can_access_post(post_id::uuid);
  ...
  USING (
    CASE
      WHEN (SELECT current_setting('request.method', true)) = 'GET' THEN
        ...
        ("postId" IS NOT NULL AND can_access_post("postId"::uuid))
